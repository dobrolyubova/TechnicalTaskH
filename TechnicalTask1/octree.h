...
class Octree
{
	...
	std::tuple<
		...
		std::vector<size_t>, 
		std::vector<Point3>
	> locate_point(Point3& p, std::shared_ptr<TreeItem<T>>& tree);
	...
	Point3 project_closest(Point3& p, std::shared_ptr<TreeItem<T>>& tree);
	...
	std::tuple<
		...
		std::vector<size_t>, 
		std::vector<Point3>>
		depth_first_search(Point3& p, Point3& p_proj, std::shared_ptr<TreeItem<T>>& tree);
